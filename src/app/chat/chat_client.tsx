...
export default function ChatClient(props) {
  ...
  const startChat = async() => {
    const chatData = await createChat(activeBot.slug);
  ...
  const selectBot = (bot) => {
    ...
    // Sort by id
    updatedBots.sort((a, b) => a.slug - b.slug);
  ...
  const loadActiveBot = (bots, activeBotSlug) => {
    let newBots = [];
    let activeBot = {}

    // Remove the new active bot from the list
    bots.forEach(bot => {
      if(bot.slug === activeBotSlug){
        activeBot = bot
      } else {
        newBots.push(bot)
      }
    ...
    // Sort by id
    newBots.sort((a, b) => a.slug - b.slug);
  ...
  const sendMessageHandler = async (e) => {
    ...
    messages.push(newCustomerMessage);
    setMessages(messages);
    setCustomerMessage("");
  }
  ...
  useEffect( () => {  
    if(props.messages.length > 0 && props.activeBotSlug != ""){
      setMessages(props.messages)
      loadActiveBot(props.bots, props.activeBotSlug)
      setChatOpen(true)
    } else {
      setBots(props.bots)
    }
    ...
    <div className="fixed bottom-4 right-4 w-80 z-50 font-sans">
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 border-b border-gray-200 relative">
          ...
          {showDropdown && (
            <div className="absolute top-full mt-[-8] left-8 bg-white shadow-md border border-gray-300  rounded-lg w-60">
              {bots.map((bot) => (
        ...
        {/* Chat body */}
        <div className="flex-1 overflow-auto p-4 space-y-2 text-sm bg-white" ref={messagesEndRef}>
        ...
        <div className="flex items-center gap-2 p-3 border-t border-gray-200 bg-gray-50">
          ...
          />
          <button className="px-3 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition text-sm shadow-sm" onClick={sendMessageHandler}>
            Send
          </button>
        </div>
      </div>
...
const botImages = {
  yoda: yodaImg,
  dark_helmet: darkHelmetImg,
  darth_vader: darthVaderImg,
};

const getImageFromBotSlug = (slug) => botImages[slug];